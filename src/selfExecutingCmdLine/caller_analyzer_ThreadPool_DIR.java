...
class Datalog_exec_TP implements Runnable {

	String fact_str,qry_str,file_path;
	public Datalog_exec_TP(String fact,String qry) 
	{
		this.fact_str=fact;
		this.qry_str=qry;
	}
	public Datalog_exec_TP(String fact,String qry,String output_filepath) 
	{
		this.fact_str=fact;
		this.qry_str=qry;
...
public class caller_analyzer_ThreadPool_DIR 
{
	public static void main(String[] args) throws Exception
	{
		...
		*/
		...
		{			
			...
			System.out.println("<Thread submit in threadpool> Executing datalog facts for :"+current_file_path);	
					
			for(int i=0;i<dp.method_count();i++)
			{
				Runnable worker=new Datalog_exec_TP(s_cp[i],qry,current_file_path.substring(0, current_file_path.indexOf(".dl")).concat("_output.txt"));
